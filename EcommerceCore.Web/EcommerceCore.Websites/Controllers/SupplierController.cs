...
namespace EcommerceCore.Web.Controllers
{
    ...
    public class SupplierController : Controller
    {
        ...
        public async Task<ActionResult> Edit(Guid? id)
        {
            ...
            var supplier = await _supplierService.FindAsyn(id.Value);
            if (supplier == null)
            {
                return HttpNotFound();
            }
        ...
        public async Task<ActionResult> Edit(SupplierViewModel supplierViewModel)
        {
            if (ModelState.IsValid)
            {
                Supplier supplier = await _supplierService.FindAsyn(supplierViewModel.Id);
                if (supplier == null)
                {
                    return HttpNotFound();
                }
        ...
        public async Task<ActionResult> Delete(Guid id)
        {
            try
            {
                var supplier = await _supplierService.FindAsyn(id);
                if (supplier == null)
                {
                    return HttpNotFound();
                }
