...
namespace EcommerceCore.Web.Controllers
{
    ...
    public class CouponController : Controller
    {
        ...
        public async Task<ActionResult> Edit(Guid? id)
        {
            ...
            var coupon = await _couponService.FindAsyn(id.Value);
            if (coupon == null)
            {
                return HttpNotFound();
            }
        ...
        public async Task<ActionResult> Edit(CouponViewModel couponViewModel)
        {
            if (ModelState.IsValid)
            {
                Coupon coupon = await _couponService.FindAsyn(couponViewModel.Id);
                if (coupon == null)
                {
                    return HttpNotFound();
                }
        ...
        public async Task<ActionResult> Delete(Guid id)
        {
            try
            {
                var coupon = await _couponService.FindAsyn(id);
                if (coupon == null)
                {
                    return HttpNotFound();
                }
