...
namespace EcommerceCore.Web.Controllers
{
    ...
    public class ManufacturerController : Controller
    {
        ...
        public async Task<ActionResult> Edit(Guid? id)
        {
            ...
            var manufacturer = await _manufacturerService.FindAsyn(id.Value);
        ...
        public async Task<ActionResult> Edit(ManufacturerViewModel manufacturerViewModel)
        {
            if (ModelState.IsValid)
            {
                Manufacturer manufacturer = await _manufacturerService.FindAsyn(manufacturerViewModel.Id);
                if (manufacturer == null)
                {
                    return HttpNotFound();
                }
        ...
        public async Task<ActionResult> Delete(Guid id)
        {
            try
            {
                var manufacturer = await _manufacturerService.FindAsyn(id);
                if (manufacturer == null)
                {
                    return HttpNotFound();
                }
