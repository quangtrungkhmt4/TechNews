...
namespace EcommerceCore.Web.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<ActionResult> Edit(Guid? id)
        {
            ...
            var product = await _productService.FindAsyn(id.Value);
            if (product == null)
            {
                return HttpNotFound();
            }
        ...
        public async Task<ActionResult> Edit(ProductViewModel productViewModel)
        {
            if (ModelState.IsValid)
            {
                Product product = await _productService.FindAsyn(productViewModel.Id);
                if (product == null)
                {
                    return HttpNotFound();
                }
        ...
        public async Task<ActionResult> Delete(Guid id)
        {
            try
            {
                var product = await _productService.FindAsyn(id);
                if (product == null)
                {
                    return HttpNotFound();
                }
