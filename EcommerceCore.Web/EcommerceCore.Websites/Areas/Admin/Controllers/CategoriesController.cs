...
namespace EcommerceCore.Websites.Areas.Admin.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public async Task<PartialViewResult> CreateOrEdit(Guid? id)
        {
            ...
            if (id != null)
            {
                var entity = await _categoryService.FindAsyn(id.Value);
                if (entity != null)
                {
                    model = Mapper.Map<CategoryViewModel>(entity);
                }
            }
            ViewBag.categoryParent = await _categoryService.FindAllAsync(c => c.IsDeleted == false);
        ...
        public async Task<ActionResult> Index(DataTableRequest request)
        {
            ...
            var categories = await _categoryService.FindAllAsync(c => c.IsDeleted == false);
        ...
        public async Task<ActionResult> Delete(Guid id)
        {
            ...
            var categories = await _categoryService.FindAsyn(id);

            categories.IsDeleted = true;

            await _categoryService.UpdateAsync(categories, id, true);
